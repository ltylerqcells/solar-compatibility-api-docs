...
Verified passing cases as of 2026-06-18T11:19:20. Each case shows the JSON request body and the verified response. Use these as reference payloads for integration.

**Base URL:** `https://solar-compatibility-api.onrender.com/`

**Total passing cases:** 96
...
```json
{
  ...
  "bom": {
    "pv": [
      {
        "sku": "Q.TRON BLK M-G2.E1+/AC 4560",
        "serial_numbers": [],
        "material_codes": [],
        "quantity": 10
      }
...
```json
{
  ...
  "components": [],
  "issues": [
    "FAIL: Module SKU 'Q.TRON BLK M-G2.E1+/AC 4560' not found in PVAVL"
  ]
}
```
...
```json
{
  ...
  "bom": {
    ...
    "battery": [
      {
        "sku": "Q.HOME CORE A7.6B13.1 US.1",
        "quantity": 1
      },
      {
        "sku": "Q.SAVE D20.0SX",
...
## ironridge_v07_crosslisting_passes

_All-IronRidge racking plus V07-A-0-E roof flashing. IronRidge is the
natural UL2703 anchor; V07-A-0-E must crosslist IronRidge. This case
exercises the crosslisting parse path â€” it should pass once _parse_csv
is fixed to split on commas rather than iterate the string character
by character.
_
...
```json
{
  ...
  "components": [
    ...
    },
    {
      ...
      "notes": [
        "Rail (XR-10-168M-US): 15.00%",
        "Mid Clamp (XR-MID-01-B1-US): 3.50%",
        "End Clamp (UFO-END-01-A1-US): 3.50%",
        "Roof Flashing/Mount/Clamp (V07-A-0-E): 0.00%",
        "MLPE Mount (BHW-MI-01-A1-US): 3.50%"
      ]
    }
  ],
  "issues": [
    "WARNING: PV-side DCA 44.4% below threshold 50%",
    "WARNING: PV-side FEOC 36.9% below threshold 40%"
  ]
}
```
...
_2028 case should return appropriate error message._

**Request:**

```json
{
  "installation-date": "2028-06-15",
  "bom": {
    "pv": [
      {
        "sku": "Q.PEAK DUO BLK ML-G10+/TS 410",
...
```json
{
  ...
  "components": [],
  "issues": [
    "FAIL: DCA and FEOC evaluation is not yet supported for installation dates after 2027-12-31 (installation date: 2028-06-15). "
  ]
}
```
...
```json
{
  ...
  "components": [],
  "issues": [
    "FAIL: DCA and FEOC requirements did not apply to installations completed before 2024 (installation date: 2023-06-15)."
  ]
}
```
...
```json
{
  ...
  "components": [],
  "issues": [
    "FAIL: DCA and FEOC requirements did not apply to installations completed before 2024 (installation date: 1990-06-15)."
  ]
}
```
...
```json
{
  ...
  "components": [],
  "issues": [
    "FAIL: DCA and FEOC evaluation is not yet supported for installation dates after 2027-12-31 (installation date: 2030-06-15). "
  ]
}
```
...
## ignore_sn_pv_bat_sn_required

_ignore_serial_validation=true: battery SKU requiring SN passes without SN_
...
```json
{
  ...
  "components": [
    ...
    },
    {
      ...
      "notes": [
        "Rail (PSR-M84): 0.00%",
        "Mid Clamp (PSR-MCZ-US): 3.50%",
        "End Clamp (PSR-HEC): 0.00%",
        "Splice (PSR-SPLS-US): 3.50%",
        "Roof Flashing/Mount/Clamp (PIF2-BDT): no DCA %",
        "L-foot / Standoff / Tilt-leg (PF-DRW85): 0.00%",
        "L-foot / Standoff / Tilt-leg (PSR-LUG): 0.00%",
        "MLPE Mount (PSR-MLP-US): 3.50%",
        "Accessories (PSR-CAP): 0.00%",
        "Accessories (PSR-CBG): 0.00%",
        "Accessories (PSR-WMC): 0.00%"
      ]
  ...
  "issues": [
    "WARNING: Rails contribute 0 to DCA \u2014 not all rails are DCA eligible",
    "WARNING: Fasteners contribute 0 to DCA \u2014 not all fasteners are DCA eligible",
    "WARNING: PV-side DCA 19.6% below threshold 50%"
  ]
}
```
...
## ignore_sn_pv_ineligible_sn_range

_ignore_serial_validation=true: ineligible PV SN range passes_
...
## ignore_sn_mc_wrong_material_code

_ignore_serial_validation=true: wrong material code passes without FAIL_
...
## ignore_sn_inv_sn_not_provided

_ignore_serial_validation=true: inverter requiring SN passes without SN_
...
```json
{
  ...
  "issues": [
    "WARNING: PV-side DCA 26.6% below threshold 50%"
  ]
}
```
...
## ignore_sn_inv_ineligible_sn

_ignore_serial_validation=true: ineligible inverter SN passes_
...
## ignore_sn_inv_sn_required_not_provided

_ignore_serial_validation=true: inverter requiring SN not provided passes_
...
## ignore_sn_battery_pooled_sn_mismatch

_ignore_serial_validation=true: unmatched pooled battery SN passes_
...
```json
{
  ...
  "components": [
    ...
    },
    {
      ...
      "notes": [
        "BatteryA (IQBATTERY-5P-1P-NA-DOM (SN begins with 54xxxx where xxxx >= 2546)): qty=1, capacity=5 kWh",
        "BatteryB (IQBATTERY-5P-1P-NA (SN begins with 54)): qty=1, capacity=5 kWh"
      ]
    }
  ],
  "issues": [
    "WARNING: ESS-side DCA 33.8% below threshold 50%",
    "WARNING: ESS-side FEOC 28.4% below threshold 55%"
  ]
}
```
...
## ignore_sn_battery_per_item_ineligible_sn

_ignore_serial_validation=true: ineligible per-item battery SN passes_
...
## ignore_sn_inverter_pooled_ineligible_sn

_ignore_serial_validation=true: ineligible pooled inverter SN passes_
...
## ignore_sn_pv_membership_not_provided

_ignore_serial_validation=true: PV membership SN not provided passes_
